         ...
         public static void ShuffleGameBoard(List<int[]> gameBoard, Tuple<int, int> gameDimensions)
        {
            ...
            for (int i = 0; i < gameDimensions.Item1; i++)
            {
                for (int j = 0; j < gameDimensions.Item2; j++)
                {
                    int row = i;
                    int col = j;
                    var img =  new PictureBox { Image = background , Parent = gamePanel, SizeMode = PictureBoxSizeMode.StretchImage,Width = gamePanel.Width/gamePanel.ColumnCount, Height = gamePanel.Width/gamePanel.ColumnCount, BorderStyle = BorderStyle.FixedSingle};
                    img.Click += (sender, e) =>
                    {
                        flip(row,col);
                    };
